...
class AnalyzeStep(Step):
    ...
    def cmd(self, macaron_cmd: str) -> list[str]:
        ...
        if sbom_file is not None:
            args.extend(["--sbom-file", sbom_file])
...
class VerifyStep(Step[VerifyStepOptions]):
    ...
    def cmd(self, macaron_cmd: str) -> list[str]:
        ...
        args.extend(["--file", self.options["policy"]])
        ...
        try:
            ...
            for include_tag in include_tags:
                if include_tag not in case_config["tags"]:
                    logger.info(
                        "Skip case '%s' because it does not have the tag '%s'",
            ...
            for exclude_tag in exclude_tags:
                if exclude_tag in case_config["tags"]:
                    logger.info(
                        "Skip case '%s' because it has the tag '%s'",
...
def do_run(
    ...
    for test_case in test_cases:
        logger.info("-" * 60)
        logger.info("Case started: '%s'.", test_case.case_dir)
        ret = test_case.run(
        ...
        if ret != 0:
            logger.error("Case failed: '%s'.", test_case.case_dir)
            return ret
        logger.info("Case passed: '%s'.", test_case.case_dir)
    return 0
